...
public class MyChartFactory {
	...
	public static JFreeChart createBarChart(String title,
		...
		// 1：创建数据集合
		DefaultCategoryDataset dataset = MyChartUtils
				.createDefaultCategoryDataset(series, categories);
		JFreeChart chart = org.jfree.chart.ChartFactory.createBarChart(title,
				categoryAxisLabel, valueAxisLabel, dataset, PlotOrientation.HORIZONTAL, true, true, false);
		// 3:设置抗锯齿，防止字体显示不清楚
		MyChartUtils.setAntiAlias(chart);// 抗锯齿
		// 4:对柱子进行渲染
		MyChartUtils.setBarRenderer(chart.getCategoryPlot(), false);//
		// 5:对其他部分进行渲染
		MyChartUtils.setXAixs(chart.getCategoryPlot());// X坐标轴渲染
		MyChartUtils.setYAixs(chart.getCategoryPlot());// Y坐标轴渲染
		// 设置标注无边框
	...
	public static JFreeChart createPieChart(String title, String[] categories,
                                            Object[] datas) {
		// 1：创建数据集合
		DefaultPieDataset dataset = MyChartUtils.createDefaultPieDataset(
				categories, datas);
		...
		MyChartUtils.setAntiAlias(chart);// 抗锯齿
		// 4:对柱子进行渲染[创建不同图形]
		MyChartUtils.setPieRender(chart.getPlot());
		/**
		 * 可以注释测试
		 */
	...
	public static JFreeChart createLineChart(String title,
		...
		MyChartUtils.setAntiAlias(chart);// 抗锯齿
		// 4:对柱子进行渲染[[采用不同渲染]]
		MyChartUtils.setLineRender(chart.getCategoryPlot(), false, true);//
		// 5:对其他部分进行渲染
		MyChartUtils.setXAixs(chart.getCategoryPlot());// X坐标轴渲染
		MyChartUtils.setYAixs(chart.getCategoryPlot());// Y坐标轴渲染
		// 设置标注无边框
	...
	public static JFreeChart createStackedBarChart(String title,
		...
		MyChartUtils.setAntiAlias(chart);// 抗锯齿
		// 4:对柱子进行渲染[创建不同图形]
		MyChartUtils.setStackBarRender(chart.getCategoryPlot());
		// 5:对其他部分进行渲染
		MyChartUtils.setXAixs(chart.getCategoryPlot());// X坐标轴渲染
		MyChartUtils.setYAixs(chart.getCategoryPlot());// Y坐标轴渲染
		// 设置标注无边框
