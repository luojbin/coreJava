...
public class ParseXMLDemo {
	public static void main(String[] args) {
		try {
			 ...
			 * 解析XML的大致步骤:
			 * 1:创建SAXReader
			 * 2:读取要解析的XML文档并生成 Document对象。 这一步就是DOM解析耗时耗资源的 地方了，因为需要将XML文档都读取 完毕并载入内存
			 * 3:通过Document对象获取根元素
...
//			reader.read(new FileInputStream("emplist.xml"));
			...
			/*
			 * 3 Document提供了获取根元素的方法 Element getRootElement() 返回的Element实例表示XML文档的根元素
			 * Element的每一个实例用于表示XML文档中的 一个元素(一对标签)
			 * Element提供了用于获取其表示的标签的信息 的相关方法:
			 *
			 * String getName() 获取当前标签的名字
			 * Element element(String name) 获取当前标签中指定名字的子标签
			 * List elements() 获取当前标签中的所有子标签
			 * List elements(String name) 获取当前标签中所有同名子标签
